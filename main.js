...
window.Splash = function () {
  ...
  this.init = function () {
    ...
    for (let i = 0; i < 2; i += 1) {
      FB.entities.push(
        new FB.BottomBar(FB.WIDTH * i, FB.HEIGHT - 100, FB.WIDTH),
      );
    }
    FB.entities.push(
      new FB.Tree(~~(Math.random() * FB.WIDTH), FB.HEIGHT - 160),
    );
    FB.entities.push(
      new FB.Tree(~~(Math.random() * FB.WIDTH + 50), FB.HEIGHT - 160),
    );
    FB.entities.push(
      new FB.Tree(~~(Math.random() * FB.WIDTH + 100), FB.HEIGHT - 160),
    );
  };
